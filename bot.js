...
client.on("message", message => {
  if (message.content === prefix + "ezan") {
    // eğer prefix ve çay yan yana kullanılırsa burda alttaki şeyleri yap
    if (message.channel.type !== "text") return;

    const { voiceChannel } = message.member;

    if (!voiceChannel) {
      // eğer kullanıcı sesli kanalda değilse resti çek
      return message.reply(
        "Lütfen #ezan sesli kanalına gir de öyle okuyum Ezanı"
      );
    }

    voiceChannel.join().then(connection => {
      // ve çal bakalım
      const stream = ytdl("https://www.youtube.com/watch?v=a9FsCJqXE0g", {
        filter: "audioonly"
      });
